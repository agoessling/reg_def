...
def wrap_comment(text: str, indent: str = '', width: int = 100) -> str:
  indent = indent + '// '
  return textwrap.fill(capitalize_sentence(text), width=width, initial_indent=indent,
...
def get_peripheral_struct(peripheral: PeripheralDefinition) -> str:
  ...
  for reg_offset, reg_list in registers.items():
    ...
    if len(reg_list) == 1:
      ...
      asserts.append(f'static_assert(offsetof({peripheral_typedef}, {reg_list[0].name.upper()})'
    ...
    # For registers which overlap, create anonymouse union.
      ...
      for register in reg_list:
        ...
        asserts.append(f'static_assert(offsetof({peripheral_typedef}, {register.name.upper()})'
